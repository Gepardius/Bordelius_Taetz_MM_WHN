...
class MultimodalBridge(nn.Module):
    ...
    def forward(self, sig_embeds, ego_embeds=None):
        # sig_embeds: [B, 729, 1152] -> [B, 729, 3584]
        vis_tokens = self.siglip_proj(sig_embeds)

        if self.use_motion and ego_embeds is not None:
            # ego_embeds: [B, 785, 768]
            # Slice the first 8 tokens: index 0 is the CLS (global motion summary),
            # indices 1-7 are early patch tokens carrying temporal structure.
            # No parameters needed — stable input from the very first training step.
            ego_sliced = ego_embeds[:, :8, :]          # [B, 8, 768]

            # Project to LLM space with the 4-layer projection
            mot_tokens = self.egovlp_proj(ego_sliced)  # [B, 8, 3584]

            # Add motion type embedding so the resampler can distinguish these
            # 8 tokens from the 729 spatial patch tokens
            mot_tokens = mot_tokens + self.motion_type_embed  # broadcasts [1,1,3584] over [B,8,3584]
...
if __name__ == '__main__':
    ...
    if TRAIN_BRIDGE:
        ...
        for epoch_full in range(total_epochs):
            ...
            progress = epoch_full / total_epochs
