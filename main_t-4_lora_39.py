...
class MultimodalBridge(nn.Module):
    ...
    def forward(self, sig_embeds, ego_embeds=None):
        # --- float32 projection to prevent rank-1 collapse ---
        # temporarily cast the projection module AND the input to float32, run the forward pass, then cast the output back to the original dtype.
...
if __name__ == '__main__':
    ...
    if TRAIN_BRIDGE:
        ...
        for epoch_full in range(total_epochs):
            ...
            # --- Anti-shortcut dropout schedule ---
            if epoch_full == 0:
                current_dropout = 0.10   # Epoch 0: small dropout — prevents text monopoly from day 1
            elif epoch_full == 1:
                current_dropout = 0.30   # Epochs 1-2: ramp up — visual pathway gets gradient
            elif epoch_full == 2:
                current_dropout = 0.30   # Epochs 3-4: peak — maximum visual forcing
            elif epoch_full == 3:
                current_dropout = 0.60
            elif epoch_full == 4:
                current_dropout = 0.60   # Epochs 5-6: taper — consolidation with mixed signal
            elif epoch_full == 5:
    ...
    if INFERENCE_BRIDGE:
        ...
        if TRAIN_BRIDGE:
            ...
            checkpoint_path = f"checkpoints/bridge_motion_True_0.1_main_t-4_lora_36_7.pt"
    ...
    if VISUALIZE_FRAMES:
        if TRAIN_BRIDGE:
            ...
            checkpoint_path = f"checkpoints/bridge_motion_True_0.1_main_t-4_lora_39_7.pt"
        ...
        os.makedirs(f"modality_visualizations/{output_file}", exist_ok=True)
        ...
        run_modality_visualizations(
            ...
            use_motion         = USE_MOTION,
            n_samples          = 5,
            save_dir           = f"modality_visualizations/{output_file}"
        )

        # ── Collapse diagnostic — paste after bridge.eval() is called ──────────────
        import os, torch
        from sklearn.decomposition import PCA
        import numpy as np

        # Find any .pt file in your cache directory
        cache_dir = "extracted_features"
        all_files = [f for f in os.listdir(cache_dir) if f.endswith(".pt")]
        sample_path = os.path.join(cache_dir, all_files[1])

        # Load and inspect — handle both raw tensor and dict formats
        raw = torch.load(sample_path, map_location="cpu")
        print(f"Cache file type: {type(raw)}")

        if isinstance(raw, dict):
            print(f"Dict keys: {list(raw.keys())}")
            # Try common key names
            for key in ["sig_features", "siglip", "vision", "features"]:
                if key in raw:
                    sig = raw[key]
                    break
            else:
                sig = list(raw.values())[0]  # fallback: first value
        elif isinstance(raw, torch.Tensor):
            sig = raw
        else:
            raise ValueError(f"Unexpected cache format: {type(raw)}")

        print(f"Raw tensor shape: {sig.shape}")
        # Expected: [729, 1152]
        # Add batch dimension for bridge
        if sig.dim() == 2:
            sig = sig.unsqueeze(0)   # [1, 729, 1152]
        elif sig.dim() == 3:
            sig = sig[:1]            # take first if already batched

        sig = sig.to("cuda", dtype=llm.dtype)

        with torch.no_grad():
            out = bridge.siglip_proj.float()(sig.float()).to(llm.dtype)  # [1, 729, 3584]

        tokens = out[0].float().cpu().numpy()  # [729, 3584]

        token_var = np.var(tokens, axis=0).mean()
        print(f"\nMean token variance across 729 tokens: {token_var:.8f}")
        print("weights (collapsed): ~0.000001 or less")
        print("weights (float32 fix trained): should be 0.001+")
        ...
        print("PC1 ~100%, rest ~0%  (collapsed)")
        print("PC1 <80%, visible spread across PCs  (healthy)")